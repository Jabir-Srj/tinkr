...
export default function HTMLMinifierPage() {
  ...
  const savings = Math.round(((1 - minified.length / html.length) * 100));
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div>
          <ToolCard title="Original HTML">
            ...
            />
            <p className="text-xs text-gray-500 mt-2">{html.length} characters</p>
          </ToolCard>
        </div>

        {/* Output */}
        <div className="space-y-4">
          <ToolCard title="Minified HTML">
            <div className="relative">
              <textarea
                value={minified}
                readOnly
                className="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-gray-100 resize-none h-72 font-mono text-sm break-all"
              />
              <button
          ...
          <ToolCard title="Stats">
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center p-3 bg-gray-100 dark:bg-slate-900 rounded">
                ...
                <span className="font-mono">{html.length - minified.length}</span>
              </div>
            </div>
          </ToolCard>
        </div>
