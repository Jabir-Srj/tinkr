...
export default function CaesarCipher() {
  ...
  const caesar = (text: string, shift: number, decode: boolean = false) => {
    const adjustedShift = decode ? (26 - (shift % 26)) % 26 : shift % 26;
    return text.replace(/[a-zA-Z]/g, (char) => {
      const start = char <= 'Z' ? 65 : 97;
      return String.fromCharCode(start + ((char.charCodeAt(0) - start + adjustedShift) % 26));
    });
  };

  const handleEncode = () => {
    const shiftNum = parseInt(shift) || 0;
    setOutput(caesar(input, shiftNum));
  };

  const handleDecode = () => {
    const shiftNum = parseInt(shift) || 0;
    setOutput(caesar(input, shiftNum, true));
  };
      ...
      <div className="min-h-screen bg-background text-foreground p-6">
        <div className="max-w-2xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">Encode and decode using Caesar cipher</p>
          </div>
          ...
          <div className="space-y-2 mb-6">
            <label className="block text-sm font-medium">Shift Value (0-25)</label>
            <input
              type="number"
              value={shift}
